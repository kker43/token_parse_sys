...
初期可以复用远程服务器上的 `/home/ubuntu/token_fetch` 项目作为外部数据生产系统。
...
## 11. 与 token_fetch 的关系

远程服务器上的 `/home/ubuntu/token_fetch` 初期建议保持独立运行。
...
当前项目不应该直接粗暴合并整个 `token_fetch` 仓库。
