...
def _fetch_quality_status_rows(
    ...
    quality_evidence: list[Mapping[str, object]] = []
    for candidate_date in candidate_dates:
        rows = _fetch_rows(
            connection,
            """
...
""",
        ...
        quality_evidence.extend(normalized)
        ready_products = {
    ...
    if selected_date is None:
        return tuple(quality_evidence)

    weekly_evidence = [
        item
        for item in quality_evidence
        if item.get("data_product") == WEEKLY_QUALITY_PRODUCT
        and str(item.get("data_date") or "") <= selected_date
    ]
    if not weekly_evidence:
...
""",
        ...
        quality_evidence.extend(
            _json_safe_mapping(row) for row in weekly_rows
        )
    return tuple(quality_evidence)
