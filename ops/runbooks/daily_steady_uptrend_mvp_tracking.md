...
例行任务必须在外部事实生产完成后运行。它通过精确日期查询从外部 `pub_data_quality_status` 只读解析最近一个日频产品完整的交易日。当前上游质量视图没有 `pub_stock_weekly_kline` 项，因此任务对已发布周线产品执行消费者侧记录数和唯一版本检查，结果只进入本次运行证据，不写回外部系统。随后自动生成并校验以下 artifact：
